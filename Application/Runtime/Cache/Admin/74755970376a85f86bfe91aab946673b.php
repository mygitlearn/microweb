...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('addProblem');?>">新 增</a>
        </div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<!-- <td><?php if(($vo["status"]) == "0"): ?>禁用 <?php else: ?>启用<?php endif; ?> </td> -->
			<!-- <td><?php echo ($vo["update_time"]); ?></td> -->
			<td><?php echo date("Y-m-d H:i", $vo['update_time']);?></td>
			<td><a href="<?php echo U('addProblem?seat='.$vo['id']);?>">编辑</a>&emsp;
			<a href="<?php echo U('changeStatus?type=security&method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get deal">删除</a>
            </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Website/security');?>');
	</script>
