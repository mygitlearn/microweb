...
<body>
	...
	<div class="cf">
		<div class="fl">
	        <a class="btn" href="<?php echo U('addColumn');?>">新 增</a>
	    </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="nickname" class="search-input" value="<?php echo I('nickname');?>" placeholder="请输入标题或者ID">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('column');?>"><i class="btn-search"></i></a>
			</div>
		</div>
    </div>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td style=""><img src="/microweb/Uploads/<?php echo ($vo["savepath"]); echo ($vo["savename"]); ?>"></td>
			<td style=""><?php echo ($vo["name"]); ?></td>
			<td style=""><?php echo ($vo["url"]); ?></td>
			<td style="">
				<img class="up" src="/microweb/Public/Admin/images/material/up.jpg" value="<?php echo ($vo["id"]); ?>">&ensp;
				<img class="down" src="/microweb/Public/Admin/images/material/down.jpg" value="<?php echo ($vo["id"]); ?>"> </td>
			<td id="tb_time"><?php echo date("Y-m-d H-i-s", $vo["update_time"]);?></td>
			<td><?php if(($vo["forbidden"]) == "0"): ?><a href="<?php echo U('changeStatus?type=topic&method=forbidUser&id='.$vo['id']);?>" class="ajax-get deal" style="color:red">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('changeStatus?type=topic&method=resumeUser&id='.$vo['id']);?>" class="ajax-get deal" style="color:green">启用</a><?php endif; ?>&emsp;
				 <a href="<?php echo U('addColumn?seat='.$vo['id']);?>">编辑</a>&emsp;
                <a href="<?php echo U('changeStatus?type=topic&method=deleteUser&id='.$vo['id']);?>" class="confirm ajax-get deal">删除</a>
            </td>
